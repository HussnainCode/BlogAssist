...
class Body extends Component {
	...
	@computed get content() {
		return this.permissions.filter(tool => tool.category === 'Rédaction')
	}

	@computed get syntax() {
		return this.permissions.filter(tool => tool.category === 'Syntaxe')
	}
	...
	return (
		...
		<>
			...
			{this.content.length ? <>
				<Title title="Vos outils de rédaction" />
				<Grid>
				...
				{this.syntax.length ? <>
					<Title title="Vos outils syntaxiques" />
					<Grid>
						{this.syntax.map((tool, index) => 
							<Tool 
								key={index}
								group={tool.category}
								title={tool.title} 
